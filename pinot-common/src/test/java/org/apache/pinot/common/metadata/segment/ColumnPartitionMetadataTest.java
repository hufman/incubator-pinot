...
public class ColumnPartitionMetadataTest {
  ...
  private static final List<Integer> PARTITIONS = Arrays.asList(1, 3, 5);
  private static final String LEGACY_METADATA_JSON =
      "{\"functionName\":\"murmur\",\"numPartitions\":10,\"partitionRanges\":\"[1 1],[3 3],[5 5]\"}";
  private static final String LEGACY_PARTITION_RANGES_STRING = "[1 1],[3 3],[5 5]";
  ...
  public void testPartitionsConfig() {
    ...
    List<Integer> actual = ColumnPartitionMetadata.extractPartitions(config.getList("partitions"));
  ...
  public void testLegacyPartitionRangesConfig() {
    ...
    List<Integer> actual = ColumnPartitionMetadata.extractPartitions(config.getList("partitionRanges"));
    assertEquals(actual, PARTITIONS);
  }
}
